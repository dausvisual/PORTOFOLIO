  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '3rem' }}>
        <div>
          <h1 style={{ fontSize: '2.2rem', marginBottom: '0.5rem' }}>Sertifikasi dan Pelatihan</h1>
          <p style={{ color: '#94a3b8' }}>Upload sertifikat dan dokumentasi pelatihan terbarumu.</p>
        </div>
        <button onClick={() => setIsAdding(!isAdding)} style={{ padding: '1rem 2rem', backgroundColor: isAdding ? '#ef4444' : '#00e5ff', color: isAdding ? '#fff' : '#020617', border: 'none', borderRadius: '0.5rem', fontWeight: 'bold', cursor: 'pointer' }}>
          {isAdding ? 'Batal Upload' : '+ Upload Sertifikat Baru'}
        </button>
      </div>
      ...
      {isAdding && (
        <div style={{ backgroundColor: '#0b0f19', padding: '2rem', borderRadius: '1rem', border: '1px dashed #00e5ff', marginBottom: '3rem' }}>
          <form style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <input type="text" placeholder="Judul/Keterangan Sertifikat..." style={inputStyle} required />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: '1.5rem' }}>
        {photos.map((photo) => (
          <div key={photo.id} style={{ backgroundColor: '#0b0f19', padding: '1rem', borderRadius: '0.8rem', border: '1px solid #1e293b' }}>
            ...
            <button style={{ width: '100%', padding: '0.6rem', backgroundColor: 'transparent', color: '#ef4444', border: '1px solid #ef4444', borderRadius: '0.4rem', cursor: 'pointer' }}>Hapus Sertifikat</button>
          </div>
        ))}
      </div>
    </div>
